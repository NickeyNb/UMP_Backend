...
export const createUser = async (body) => {
    try {
        if (!body)
            return responseData(400, "Invalid JSON body");
...
export const getAllUsers = async () => {
    try {
        const users = await dynamodbClient.send(new ScanCommand({
            TableName: config.userTable,
        }));
...
export const getUser = async (email) => {
    try {
        if (!email)
            return responseData(400, "Invalid credential");
...
export const updateUser = async ({ email, username, age, role, }) => {
    try {
        if (!email)
            return responseData(400, "Invalid credential");
        console.log(email, username, age);
        const res = await dynamodbClient.send(new UpdateCommand({
            TableName: config.userTable,
            Key: { email },
            UpdateExpression: "SET #username=:username, #age=:age, #updatedAt=:updatedAt, #role=:role",
...
export const deleteUser = async (email) => {
    try {
        if (!email)
            return responseData(400, "Invalid credential");
        const res = await dynamodbClient.send(new DeleteCommand({
            TableName: config.userTable,
            Key: { email },
            ConditionExpression: "attribute_exists(email)",
        }));
